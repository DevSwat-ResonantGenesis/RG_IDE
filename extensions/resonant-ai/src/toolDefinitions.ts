...
const TOOLS_VISUALIZER: ToolDef[] = [
  ...
  { type: F, function: { name: 'code_visualizer_compare', description: 'Compare 2-3 codebases: file drift, breaking changes, evolution timeline, instability metrics, heat maps, modularity score.', parameters: { type: 'object', properties: { paths: { type: 'string', description: 'Comma-separated absolute paths (e.g. "/proj/v1,/proj/v2")' }, labels: { type: 'string', description: 'Comma-separated labels (e.g. "v1,v2")' } }, required: ['paths'] } } },
  { type: F, function: { name: 'code_visualizer_live_nodes', description: 'Get all live (reachable from execution roots) nodes from governance analysis.', parameters: { type: 'object', properties: { path: { type: 'string' }, drift_threshold: { type: 'number' } }, required: ['path'] } } },
  { type: F, function: { name: 'code_visualizer_invalid_nodes', description: 'Get all invalid/dead (unreachable) nodes from governance analysis.', parameters: { type: 'object', properties: { path: { type: 'string' }, drift_threshold: { type: 'number' } }, required: ['path'] } } },
  { type: F, function: { name: 'code_visualizer_compile', description: 'Compile a GAL (Graph Analysis Language) action into a reversible patch artifact. Pure, deterministic, auditable. Actions: TAG_SUBGRAPH, ISOLATE_SUBGRAPH, PROPOSE_DELETE_SUBGRAPH, MARK_EXECUTION_ROOT.', parameters: { type: 'object', properties: { path: { type: 'string' }, gal_action: { type: 'string', description: 'JSON string: {"action_type":"TAG_SUBGRAPH","target_node":"node_id","params":{"tag":"deprecated"}}' } }, required: ['path', 'gal_action'] } } },
  { type: F, function: { name: 'code_visualizer_verify_invariants', description: 'Verify formal safety invariants on the codebase graph: execution root preservation, root connectivity, no orphaned roots. Returns proofs, not booleans.', parameters: { type: 'object', properties: { path: { type: 'string' } }, required: ['path'] } } },
];
...
const TOOLS_DEPLOY: ToolDef[] = [
  ...
  { type: F, function: { name: 'ssh_run', description: 'Run a non-interactive SSH command on a remote host (uses system ssh). Requires SSH keys already configured on this machine. Use for droplet checks and deploy steps.', parameters: { type: 'object', properties: { host: { type: 'string', description: 'Host or IP address' }, user: { type: 'string', description: 'SSH user (e.g. "deploy")' }, port: { type: 'number', description: 'SSH port (default 22)' }, command: { type: 'string', description: 'Remote shell command to run' } }, required: ['host', 'user', 'command'] } } },
  ...
  { type: F, function: { name: 'droplet_deploy_frontend', description: 'Deploy frontend on a remote droplet using git pull + build + rsync + nginx reload flow. Requires SSH keys configured on this machine.', parameters: { type: 'object', properties: { host: { type: 'string' }, user: { type: 'string' }, repo_dir: { type: 'string', description: 'Repo working directory on droplet' }, web_root: { type: 'string', description: 'Nginx web root to rsync into' }, branch: { type: 'string', description: 'Branch name (default main)' } }, required: [] } } },
];
...
// All available tool definitions — sent to server so it knows what tools can be executed locally
